# -*- coding: utf-8 -*-
"""
Created on Fri Feb 17 13:56:54 2023
...
root_1b = (-b+np.sqrt(discriminate))/2*a
root_2b = (-b-np.sqrt(discriminate))/2*a
print()
...
M= (w*(-6*x**2+6*L*x-L**2))/12
print()
print('(d) The bending moment at each step of the array is {0}'.format(M))
...
Let the maximum bendingmoment be max_M and the minimum bending moment be min_M
...
When the bendimgis max_M,we get an expression x**2-Lx + (L**2/6)+(2*max_M)/w=0
